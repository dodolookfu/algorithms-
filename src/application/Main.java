...
public class Main extends Application {
	...
	public void plan(int[] centerpoint,ArrayList<LBG9> data9)
	{
		 ...
		 {
			 center.add(new LBG4(data.get(centerpoint[a11]).Or,data.get(centerpoint[a11]).Og,data.get(centerpoint[a11]).Ob));
	...
	public void clustering()
	{
		...
		for (int i=0;i<codebook;i++)
		 ...
		 if(oldtest==0){
			for(int i=0;i<data8.size();i++){   //MSE= (Xr-Zr)^2 + (Xg-Zg)^2 + (Xb-Zb)^2 / p
