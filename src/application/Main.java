...
public class Main extends Application {
	...
	//display newimage
	public void resetChoose(ArrayList<RGB_group> data2,float [][]newpalette)
	{
		int codebook = Integer.parseInt(codeBook.getText());
		...
		for(int a=0;a<data2.size();a++)
		{
			...
			{
				float rr = (data2.get(a).Or-newpalette[b][0])*(data2.get(a).Or-newpalette[b][0]);
				float gg = (data2.get(a).Og-newpalette[b][1])*(data2.get(a).Og-newpalette[b][1]);
				float bb = (data2.get(a).Ob-newpalette[b][2])*(data2.get(a).Ob-newpalette[b][2]);
				float calculator = (float) Math.sqrt(rr+gg+bb);
				if(calculator<minVaule)
				{
					minVaule = calculator;
					data2.get(a).group = b;
					data2.get(a).Nr =(int) (newpalette[b][0]);
					data2.get(a).Ng =(int) (newpalette[b][1]);
					data2.get(a).Nb =(int) (newpalette[b][2]);
				}
			}
		}
		...
		for(int x=0;x<width;x++)
		{
			...
			{
				int point = (x*hight)+y;
				int r=(int)(Math.round((data2.get(point).Nr)<<16));
				int g=(int)(Math.round((data2.get(point).Ng)<<8));
				int b=(int)(Math.round(data2.get(point).Nb));
				int rgb = r+g+b;
	...
	public void initial_seed(int[] centerpoint,ArrayList<Color_info> noRepeatAry)
	{
		...
		for(int b=0;b<codebook;b++){
			...
			if(b>0){
				for(int x=0;x<b;x++){
					...
					if(data.get(centerpoint[b]).Or==data.get(centerpoint[x]).Or && data.get(centerpoint[b]).Og==data.get(centerpoint[x]).Og && data.get(centerpoint[b]).Ob==data.get(centerpoint[x]).Ob){
						b--;
					}
				}
			}
		...
		for(int i=0;i<codebook;i++){
			center.add(new Codebook(data.get(centerpoint[i]).Or,data.get(centerpoint[i]).Og,data.get(centerpoint[i]).Ob));
			System.out.println(center.get(i).Or+" "+center.get(i).Og+" "+center.get(i).Ob);
		}
	}

	//表格用的顏色計算
	public void APPCOLOR(int w,ArrayList<NoRepeat_color> noRepeatColor,ArrayList<RGB_group> data2,float [][]newpalette)
	{
		int codebook = Integer.parseInt(codeBook.getText());
		...
		for(int a=0;a<data2.size();a++)
		{
			...
			{
				//RED
				float rr = (data2.get(a).Nr-COLOR[b][0])*(data2.get(a).Nr-COLOR[b][0]);
				float gg = (data2.get(a).Ng-COLOR[b][1])*(data2.get(a).Ng-COLOR[b][1]);
				float bb = (data2.get(a).Nb-COLOR[b][2])*(data2.get(a).Nb-COLOR[b][2]);
				float calculator = (float) Math.sqrt(rr+gg+bb);
				if(calculator<minVaule)
				{
					minVaule = calculator;
	...
	public void clustering()
	{
		int k=0,l=0,wr=0;
		int codebook = Integer.parseInt(codeBook.getText());
		float[][] total = new float[codebook][4];
		newpalette = new float[codebook][4];
		float [][]oldpalette = new float[codebook][4];
        int oldchartBtn=0;
        float NEWMSE=0,OLDMSE=0;
		...
		for(int a=0;a<data.size();a++)
		{
			...
			total[data.get(a).group][0]+=data.get(a).Or;
			total[data.get(a).group][1]+=data.get(a).Og;
			total[data.get(a).group][2]+=data.get(a).Ob;
			total[data.get(a).group][3]+=1;
		}

		for (int i=0;i<codebook;i++)
	     {
			 oldpalette[i][0] = newpalette[i][0];
	         oldpalette[i][1] = newpalette[i][1];
	         oldpalette[i][2] = newpalette[i][2];
	         oldpalette[i][3] = newpalette[i][3];
	         newpalette[i][0] =(int)Math.round(total[i][0]/total[i][3]);
	         newpalette[i][1] =(int)Math.round(total[i][1]/total[i][3]);
	         newpalette[i][2] =(int)Math.round(total[i][2]/total[i][3]);
	     }

		 OLDMSE=NEWMSE;

		 if(oldchartBtn==0){
			for(int i=0;i<noRepeatColor.size();i++){   //MSE= (Xr-Zr)^2 + (Xg-Zg)^2 + (Xb-Zb)^2 / p
				double rr= Math.pow(data.get(i).Or-oldpalette[data.get(i).group][0], 2);
				double gg = Math.pow(data.get(i).Og-oldpalette[data.get(i).group][1], 2);
				double bb = Math.pow(data.get(i).Ob-oldpalette[data.get(i).group][2], 2);
	            OLDMSE += (rr+gg+bb);
	        }
			OLDMSE=OLDMSE/data.size();
			oldchartBtn++;
		 }

		 NEWMSE = 0;
		 	for(int i =0;i<noRepeatColor.size();i++){   //MSE= (Xr-Zr)^2 + (Xg-Zg)^2 + (Xb-Zb)^2 / p
	            double rr=Math.pow(data.get(i).Or-newpalette[data.get(i).group][0], 2);
	            double gg=Math.pow(data.get(i).Og-newpalette[data.get(i).group][1], 2);
	            double bb=Math.pow(data.get(i).Ob-newpalette[data.get(i).group][2], 2);
	            NEWMSE +=(rr+gg+bb);
	        }
			NEWMSE=NEWMSE/data.size();

			if((Math.abs(OLDMSE - NEWMSE))/NEWMSE<num){
                k=1;
                System.out.println("OLD:"+OLDMSE+" "+"NEW:"+NEWMSE+" "+(Math.abs(OLDMSE - NEWMSE))/NEWMSE);
		...
		label_counter.setText("執行次數："+l+" 次");
	}
